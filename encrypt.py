...
def embed_data(image, data_bits):
    ...
    if len(data_bits) > len(img_flat):
        raise ValueError("Data too large to embed in this image!")

    # Ensure uint8 range (0-255)
    img_flat[:len(data_bits)] = (img_flat[:len(data_bits)] & 254) | np.array(data_bits, dtype=np.uint8)
...
def encrypt():
    ...
    if not os.path.exists(img_path):
        messagebox.showerror("Error", "Input image 'hacker.jpg' not found!")
        return
    image = cv2.imread(img_path)
    if image is None:
        messagebox.showerror("Error", "Failed to load 'hacker.jpg'.")
        return
